...
static inline VALUE method_missing(VALUE obj, ID id, int argc,
                                   const VALUE *argv, int call_status);
...
static int vm_backtrace_each(rb_thread_t *th, int lev, void (*init)(void *),
                             rb_backtrace_iter_func *iter, void *arg);
static NODE *vm_cref_push(rb_thread_t *th, VALUE klass, int noex,
                          rb_block_t *blockptr);
static VALUE vm_exec(rb_thread_t *th);
static void vm_set_eval_stack(rb_thread_t *th, VALUE iseqval, const NODE *cref);
static int vm_collect_local_variables_in_heap(rb_thread_t *th, VALUE *dfp,
                                              VALUE ary);
...
static inline VALUE vm_call0(rb_thread_t *th, VALUE recv, VALUE id, int argc,
...
again:
  ...
  case VM_METHOD_TYPE_CFUNC: {
    EXEC_EVENT_HOOK(th, RUBY_EVENT_C_CALL, recv, id, klass);
    {
      ...
      rb_control_frame_t *cfp =
          vm_push_frame(th, 0, VM_FRAME_MAGIC_CFUNC, recv, (VALUE)blockptr, 0,
                        reg_cfp->sp, 0, 1);

      cfp->me = me;
      val = call_cfunc(def->body.cfunc.func, recv, def->body.cfunc.argc, argc,
                       argv);
  ...
  case VM_METHOD_TYPE_MISSING: {
    ...
    return rb_funcall2(recv, idMethodMissing, argc + 1, RARRAY_PTR(new_args));
  }
...
static inline VALUE vm_call_super(rb_thread_t *th, int argc,
                                  const VALUE *argv) {
...
static inline int rb_method_call_status(rb_thread_t *th, rb_method_entry_t *me,
 ...
 * \param scope
 * \param self   self in the caller. Qundef means the current control frame's
 *self.
 *
 * \note \a self is used in order to controlling access to protected methods.
 */
static inline VALUE rb_call0(VALUE recv, ID mid, int argc, const VALUE *argv,
                             call_type scope, VALUE self) {
...
static VALUE check_funcall_exec(struct rescue_funcall_args *args) {
  ...
  return rb_funcall2(args->recv, idMethodMissing, args->argc + 1,
                     RARRAY_PTR(new_args));
}
...
static VALUE check_funcall(VALUE recv, ID mid, int argc, VALUE *argv) {
  ...
  if (call_status != NOEX_OK) {
    if (rb_method_basic_definition_p(CLASS_OF(recv), idMethodMissing)) {
      ...
      args.argc = argc;
      args.argv = argv;
      return rb_rescue2(check_funcall_exec, (VALUE)&args, check_funcall_failed,
...
#define type_case(t)                                                           \
  case t:                                                                      \
    return #t;
  switch (type) {
    type_case(T_NONE) type_case(T_OBJECT) type_case(T_CLASS) type_case(T_MODULE)
        type_case(T_FLOAT) type_case(T_STRING) type_case(T_REGEXP)
            type_case(T_ARRAY) type_case(T_HASH) type_case(T_STRUCT)
                type_case(T_BIGNUM) type_case(T_FILE) type_case(T_DATA)
                    type_case(T_MATCH) type_case(T_COMPLEX)
                        type_case(T_RATIONAL) type_case(T_NIL) type_case(T_TRUE)
                            type_case(T_FALSE) type_case(T_SYMBOL)
                                type_case(T_FIXNUM) type_case(T_UNDEF)
                                    type_case(T_NODE) type_case(T_ICLASS)
                                        type_case(T_ZOMBIE) default
        : return NULL;
  }
#undef type_case
}
...
NORETURN(static void raise_method_missing(rb_thread_t *th, int argc,
...
static void raise_method_missing(rb_thread_t *th, int argc, const VALUE *argv,
                                 VALUE obj, int last_call_status) {
  ID id;
...
void rb_raise_method_missing(rb_thread_t *th, int argc, VALUE *argv, VALUE obj,
                             int call_status) {
  th->passed_block = 0;
  raise_method_missing(th, argc, argv, obj, call_status | NOEX_MISSING);
}
...
static VALUE eval_string(VALUE self, VALUE src, VALUE scope, const char *file,
                         int line) {
...
rb_f_eval(int argc, VALUE *argv, VALUE self) {
  ...
  if (rho_ruby_is_enabled_eval() == 0 &&
      (sockStr == NULL || strstr(sockStr, "TCPSocket") == NULL)) {

    rb_raise(rb_eNotImpError, "Not implemented: eval is not supported.");
  } else {
    // RHO
...
rb_eval_cmd(VALUE cmd, VALUE arg, int level) {
  ...
  rb_set_safe_level_force(safe);
  if (state)
    rb_vm_jump_tag_but_local_jump(state, val);
  return val;
}
...
static VALUE yield_under(VALUE under, VALUE self, VALUE values) {
  ...
  if (values == Qundef) {
    ...
    return vm_yield_with_cref(th, RARRAY_LENINT(values), RARRAY_PTR(values),
                              cref);
  }
}

/* string eval under the class/module context */
static VALUE eval_under(VALUE under, VALUE self, VALUE src, const char *file,
                        int line) {
...
static VALUE specific_eval(int argc, VALUE *argv, VALUE klass, VALUE self) {
  if (rb_block_given_p()) {
    ...
    if (rho_is_eval_disabled())
      rb_raise(rb_eNotImpError,
               "Not implemented: only eval of block is supported.");
    // RHO
...
VALUE
rb_make_backtrace(void) { return vm_backtrace(GET_THREAD(), -1); }
...
void Init_vm_eval(void) {
  ...
  rb_add_method(rb_cBasicObject, rb_intern("__send__"),
                ...
                0);
  rb_add_method(rb_mKernel, rb_intern("send"), VM_METHOD_TYPE_OPTIMIZED,
                (void *)OPTIMIZED_METHOD_TYPE_SEND, 0);
#else
